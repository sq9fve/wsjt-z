  ...
  m_block_udp_status_updates {false}
{
  ...
  connect(ui->pte_prefixFilter, &QPlainTextEdit::textChanged,
          this, &MainWindow::invalidateFilterCache);
  connect(ui->pte_stateFilter, &QPlainTextEdit::textChanged,
          this, &MainWindow::invalidateFilterCache);
...
// m_settings->setValue("ShMsgs",m_bShMsgs);
  ...
  m_settings->setValue ("infoMessageShown12-2024", m_infoMessageShown);
...
void MainWindow::readSettings()
{
  ...
  // FT8 thread count restore (0 = Auto, 1..12 = pinned)
  ...
  switch (m_ft8threads) {
    case 0:  ui->actionMTAuto->setChecked(true); break;
    case 1:  ui->actionMT1->setChecked(true);    break;
    case 2:  ui->actionMT2->setChecked(true);    break;
    case 3:  ui->actionMT3->setChecked(true);    break;
    case 4:  ui->actionMT4->setChecked(true);    break;
    case 5:  ui->actionMT5->setChecked(true);    break;
    case 6:  ui->actionMT6->setChecked(true);    break;
    case 7:  ui->actionMT7->setChecked(true);    break;
    case 8:  ui->actionMT8->setChecked(true);    break;
    case 9:  ui->actionMT9->setChecked(true);    break;
...
void MainWindow::setDecodedTextFont (QFont const& font)
{
  ...
  if (m_qsymonitorWidget) {
    m_qsymonitorWidget->changeFont (font);
  }
  updateGeometry ();
}
...
// Get power, spectrum, and ihsym
  ...
  // MTD-aware early-decode guards (mirror wsjtx-orig lines 2360-2361).
  // ndecoderstart is hardcoded to 3 (Normal), so (MTD && start!=1) == MTD.
  // Without these guards, the early decoder pass runs in addition to the final
...
void MainWindow::on_contest_log_action_triggered()
{
  ...
  // UniqueConnection prevents duplicate connections each time the menu action is triggered;
  // without it, set_nQSO() fires N times after N opens, eventually hitting a stale receiver.
  connect(m_logBook.contest_log(), &CabrilloLog::qso_count_changed,
          m_contestLogWindow.data(), &CabrilloLogWindow::set_nQSO,
          Qt::UniqueConnection);
  m_contestLogWindow->set_nQSO(m_logBook.contest_log()->n_qso());
}
...
void MainWindow::on_actionQSYMessage_Creator_triggered()
{
  if (!m_QSYMessageCreatorWidget) {
    ...
    connect(m_QSYMessageCreatorWidget.data(), &QSYMessageCreator::sendMessage,
            this, &MainWindow::update_tx5);
  }

  m_QSYMessageCreatorWidget->setWindowFlags(
      m_QSYMessageCreatorWidget->windowFlags() | Qt::WindowStaysOnTopHint);
            ...
            if (match.hasMatch()) {
              ...
              if (the_call == qCall && ui->actionEnable_QSY_Popups->isChecked()) {
                if (m_QSYMessageWidget) {
                  m_QSYMessageWidget->write_settings();
                }
                ...
                connect(m_QSYMessageWidget.data(), &QSYMessage::sendReply,
                        this, &MainWindow::reply_tx5,
                        static_cast<Qt::ConnectionType>(Qt::UniqueConnection));
                m_QSYMessageWidget->setWindowFlags(
                    m_QSYMessageWidget->windowFlags() | Qt::WindowStaysOnTopHint);
                m_QSYMessageWidget->show();
                m_QSYMessageWidget->raise();
                m_QSYMessageWidget->activateWindow();
              }
              if (m_qsymonitorWidget && !finalMatch.isEmpty() && finalMatch.mid(0, 1) != 'Z') {
                m_qsymonitorWidget->getQSYData(QString(bhList[0]) + " " + the_call + " " + finalMatch);
              }
            }
          }
        }
      }
    } else if (((the_line.mid(22).contains(qDXCall + QString(".") + "OKQSY")
                 || the_line.mid(22).contains(qDXCall + QString(".") + "NOQSY")))
                && ui->actionEnable_QSY_Popups->isChecked()) {
      QString yesOrNo = the_line.contains("OKQSY") ? QString(" OKQSY") : QString(" NOQSY");
      on_stopTxButton_clicked();
...
void MainWindow::decode()                                       //decode()
{
  ...
  dec_data.params.ntrials10 = 0;
  dec_data.params.ntrialsrxf10 = 0;
  dec_data.params.nprepass = 4;               // preamble passes
  dec_data.params.nharmonicsdepth = 0;
...
//  pause_jt9 ();
  ...
  if((m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2")
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  ...
  QString all_decodes;
  if(m_ActiveStationsWidget!=NULL) {
    bDisplayPoints=(m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2") and
      (m_specOp==SpecOp::ARRL_DIGI or m_ActiveStationsWidget->isVisible());
  }
  // Per-call config cache. my_call etc. were called 18+ times
  // inside the per-line loop; each returned a fresh QString temporary. Caching
  // here cuts ~30 string allocations per decoded line (>1000 per FT8 cycle).
  auto const my_call = m_config.my_callsign();
  auto const my_grid = m_config.my_grid();
  auto const dxcc    = m_config.DXCC();
  ...
  for (auto line_read : lines_to_process) {
    ...
    if (ui->actionEnable_QSY_Popups->isChecked() || m_qsymonitorWidget) {
      showQSYMessage(QString::fromUtf8(line_read.constData()));
    }
    ...
    if (m_mode.startsWith ("FST4"))
      {
        ...
        auto const& cs = my_call.toLocal8Bit ();
...
//      if(m_mode=="FT8" && fTR>0.6 && fTR<0.75) decodeDone();  // Clear a hung decoder status
      ...
      auto for_us  = decodedtext.string().contains(" " + my_call + " ") or
              ...
              decodedtext.string().contains(m_baseCall+" ") or
              decodedtext.string().contains(" <" + my_call + "> ");
...
//Left (Band activity) window
      if(!bAvgMsg) {
        if(m_mode=="FT8" and SpecOp::FOX == m_specOp) {
          ...
          if ((m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2") and bDisplayPoints and decodedtext1.isStandardMessage()) {
...
#ifdef FOX_OTP
          if ((SpecOp::HOUND == m_specOp) &&
               ((sfox && (decodedtext0.mid(24, 8) == "$VERIFY$")) || // $VERIFY$ K8R 920749
...
#ifndef FOX_OTP
              if (m_specOp==SpecOp::HOUND && sfox && (decodedtext0.mid(4,2).contains("00") or decodedtext0.mid(4,2).contains("30"))) verified = false;
#endif
            }
          ...
          if (!isFiltered || for_us)
          {
              ...
              if ((m_config.showDistance() || m_config.showBearing())&& grid.contains(grid_regexp)) {
                  ...
                  if (m_config.showDistance()) {
                      ...
                      if(miles) distance += "mi";
                      if(!miles) distance += "km";
                  }
...
//Right (Rx Frequency) window
      ...
      if(m_mode=="FT8" or m_mode=="FT4" or m_mode=="FT2" or m_mode=="FST4" or m_mode=="Q65") {
        int ftol=10;
...
// multi-station special events.
      ...
      if(m_mode=="FT8" and SpecOp::HOUND==m_specOp) {
        if(decodedtext.string().contains(";")) {
          ...
          if((w.at(2)==my_call or w.at(2)==Radio::base_callsign(my_call))
          ...
          if(w.size()>=3) {
            ...
            if((w.at(0)==my_call or w.at(0)==Radio::base_callsign(my_call)) and
              ...
              if(w.at(2)=="RR73") {
                ...
                if(w.at(1)==Radio::base_callsign(ui->dxCallEntry->text()) and
                  ...
                  if (text.contains(" " + my_call + " " + m_hisCall) && !text.contains("73 "))  processMessage(decodedtext0);   // needed for MSHV multistream messages
                }
              }
            }
          }
...
void MainWindow::auto_sequence (DecodedText const& message, unsigned start_tolerance, unsigned stop_tolerance)
{
  ...
  if (message_words.size () > 3 && (message.isStandardMessage() || (is_73 or is_OK))) {
               ...
               )
    {
        ...
        if (m_zdebug) log(": autoButton: " + QString::number(ui->autoButton->isChecked()));
        if (m_zdebug) log(": cbAutoCall: " + QString::number(ui->cbAutoCall->isChecked()));
        if (m_zdebug) log(": cbnAutoCQ: " + QString::number(ui->cbAutoCQ->isChecked()));
        if (m_zdebug) log(": m_priorityCall: " + m_priorityCall);
        if (m_zdebug) log(": dxCallEntry: " + ui->dxCallEntry->text());
        if (m_zdebug) log(": m_hiscall: " + m_hisCall);
...
void MainWindow::guiUpdate()
{
  ...
  // Per-tick caches. guiUpdate runs every 100ms (10x/sec) and called
  // sfox 8x and now 6x per tick.
  bool const sfox = m_config.superFox();
  auto const now  = QDateTime::currentDateTimeUtc();
  txDuration=tx_duration(m_mode,m_TRperiod,m_nsps,m_bFast9);
  if(m_mode=="FT8" and m_specOp==SpecOp::FOX and sfox) txDuration=1.0+151*1024.0/12000.0;
  // qDebug () << "DEBUG SF " << m_mode << m_TRperiod << m_nsps << (SpecOp::FOX==m_specOp) << sfox << txDuration;
  double tx1=0.0;
  double tx2=txDuration;
  if(m_mode=="FT8" or m_mode=="FT4" or m_mode=="FT2") icw[0]=0;              //No CW ID in FT4 or FT8 mode
...
// Don't allow Fox mode in any of the default FT8 sub-bands.
    ...
    if(g_iptt==0 and ((m_bTxTime and (fTR < 0.75) and (msgLength>0)) or m_tune)) {
      ...
      if(m_mode=="FT8") {
        ...
        else if (SpecOp::HOUND == m_specOp && !sfox) {
          if(m_auto && !m_tune) {
...
//----------------------------------------------------------------------
    ...
    if(m_tune or m_mode=="Echo") {
      ...
      if(m_mode=="WSPR") genwspr_(message, msgsent, const_cast<int *> (itone),
                                    (FCL)22, (FCL)22);
      if(m_mode=="MSK144" or m_mode=="FT8" or m_mode=="FT4" or m_mode=="FT2"
    ...
    if(m_restart) {
      ...
      if (m_config.TX_messages () and m_mode!="Echo") {
        ui->decodedTextBrowser2->displayTransmittedText(m_currentMessage.trimmed(),m_mode,
                     ui->TxFreqSpinBox->value(),m_bFastMode,m_TRperiod,sfox);
        // Z
        if (m_zdebug) log("TX message: " + m_currentMessage.trimmed());
        }
    }

    auto t2 = now.toString ("hhmm");
    icw[0] = 0;
        ...
        if (m_config.TX_messages () && !m_tune && SpecOp::FOX!=m_specOp)
          {
            ui->decodedTextBrowser2->displayTransmittedText(current_message.trimmed(),
                  m_mode,ui->TxFreqSpinBox->value(),m_bFastMode,m_TRperiod,sfox);
            if (m_zdebug) log("TX message: " + current_message.trimmed());
          }
      }
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if(!m_bFastMode and (!m_config.enable_VHF_features() or m_mode=="FT8" or m_mode=="FT4" or m_mode=="FT2" or m_mode=="FST4")) {
...
void MainWindow::genCQMsg ()
{
  ...
  if(my_callsign.size () && m_config.my_grid().size ()) {
    ...
    if((m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2" or m_mode=="MSK144" || "Q65" == m_mode) and
...
//### Check this stuff ###
  ...
  if((m_mode=="MSK144" or m_mode=="FT8" or m_mode=="FT4" or m_mode=="FT2" || "Q65" == m_mode) and
...
void MainWindow::transmit (double snr)
{
  ...
  if((m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2") and m_maxPoints>0 and SpecOp::ARRL_DIGI==m_specOp) {
    ui->dxCallEntry->setText(m_deCall);
    ui->dxGridEntry->setText(m_deGrid);
    genStdMsgs("-10");
  }
...
// In auto-sequencing mode, stop after 5 transmissions of "73" message.
  if (m_bFastMode || m_bFast9) {
    if (ui->cbAutoSeq->isEnabled () && ui->cbAutoSeq->isChecked ()) {
      if(m_ntx==5) {
        m_nTx73 += 1;
      } else {
        m_nTx73=0;
...
void MainWindow::p1ReadFromStdout()                        //p1readFromStdout
{
  ...
  while(p1.canReadLine()) {
    ...
    if(t.indexOf("<DecodeFinished>") >= 0) {
      ...
      t=t.mid(0,n-2) + "                                                  ";
      t = t.trimmed();
      QStringList rxFields = t.simplified().split(QChar(' '), SkipEmptyParts);
      QString rxLine;
      QString grid="";
      if ( rxFields.count() == 8 ) {
...
{
  if (m_ActiveStationsWidget != NULL and (m_mode == "Q65" or m_mode=="FT4" or m_mode=="FT8" or m_mode=="FT2")) {
    if (m_specOp == SpecOp::Q65_PILEUP) {
...
// File-scope compiled regex objects: built once at process start, reused per call.
// Construction of QRegularExpression includes JIT compile (PCRE2); doing it once
// instead of per-decode is a major win.
...
bool MainWindow::callsignFiltered(DecodedText dt)
{
    ...
    if( message_words.size() > 3 && ( ui->cb_ignoreCQTarget->currentIndex() > 0 || ui->cb_filter_CQDX_Continent->currentIndex() > 0) && message_words[2].startsWith("CQ")) {
        ...
        if (w2.length()<=4 && !w2.contains(kReDigit)) {
            CQTarget = w2;
        } else {
            CQTarget = w1.mid(w1.indexOf(" ")+1, -1);
        }

        if (m_zdebug) log("++ CQ Target: " + CQTarget);
        if ((CQTarget == "CQ" || CQTarget == "DX") && ui->cb_ignoreCQTarget->currentIndex() == 3) return true;
            ...
            if (CQTarget != "CQ")
            {
                ...
                if (CQTarget == "DX" && ui->cb_filter_CQDX_Continent->currentIndex() > 0 ) {
                    if (m_zdebug) log("callsignFiltered: CQ DX filtering...");
    ...
    if (ui->cb_autoCallNext->isChecked()) {
        if (m_zdebug) log("callsignFiltered: Pounce mode. Exiting.");
        return false;
    }
...
void MainWindow::changeEvent(QEvent* event)
{
    if (event->type() == QEvent::WindowStateChange) {
        ...
        if (windowState() == Qt::WindowNoState &&
            stateChangeEvent->oldState() & Qt::WindowMinimized) {
            if (!ui->cbDockWF->isChecked()) return;
            if (m_wideGraph) {
                m_wideGraph->show();
            }
...
void MainWindow::qrzInit() {
    ...
    if (m_zdebug) log("EXIT: qrzInit");
}

void MainWindow::qrzSetSessionKey(QNetworkReply *r) {
    if (m_zdebug) log("ENTRY: qrzSetSessionKey");
    ...
    r->deleteLater();
    if (m_zdebug) log("EXIT: qrzSetSessionKey");
}
...
void MainWindow::log(QString s) {
    if (!m_zdebug) return ;   // cached; updated by actionWSJT_Z_Debug::toggled signal
...
void MainWindow::toggleBands() {
  ...
  if (newBand.isEmpty() || row < 0) return;
    ...
    if (m_zdebug) log("toggleBands: EXIT");
...
void MainWindow::ZProcess ()
{
    ...
    if (m_config.ignoreListReset() != 0 && m_ignoreListReset.secsTo(QDateTime::currentDateTime()) / 60 >= m_config.ignoreListReset()) {
        if (m_zdebug) log("ZProcess: Clearing ignore list");
        on_btn_clearIgnore_clicked();
    }
    ...
    if (m_zdebug) log("ZProcess: autoButton: " + QString::number(ui->autoButton->isChecked()));
    if (m_zdebug) log("ZProcess: cbAutoCall: " + QString::number(ui->cbAutoCall->isChecked()));
    if (m_zdebug) log("ZProcess: cbnAutoCQ: " + QString::number(ui->cbAutoCQ->isChecked()));
    if (m_zdebug) log("ZProcess: m_priorityCall: " + m_priorityCall);
    if (m_zdebug) log("ZProcess: dxCallEntry: " + ui->dxCallEntry->text());
...
QString MainWindow::stateLookup(QString callsign) {
    ...
    // Use a named connection so we don't clobber the default SQLite connection
    // (set up in main.cpp for the contest log). The previous code called
    // addDatabase("QSQLITE") with no name, which replaces the unnamed default
    // connection — after the first US callsign decode, every contest log query
    // would fail with "No query / Unable to fetch row" and add_QSO would
    // propagate a std::runtime_error out of the Qt slot, crashing the app.
    static const QString kConnName = QStringLiteral("usstate");
    QSqlDatabase db;
    if (QSqlDatabase::contains(kConnName)) {
        db = QSqlDatabase::database(kConnName);
    } else {
        auto const file_name = "USState.db";
        QString path = m_config.data_dir ().absoluteFilePath (file_name);
        db = QSqlDatabase::addDatabase("QSQLITE", kConnName);
        db.setDatabaseName(path);
        if (!db.open()) {
            qDebug() << "USState: connection failed:" << db.lastError().text();
            if (m_zdebug) log("USState DB: connection failed");
            return "";
        }
    }
    if (!db.isOpen()) return "";
...
void MainWindow::logSlots() {
    if (m_zdebug) log("SLOTS========================");
    for (int x = 0; x < busySlots.size(); x++) {
        QVector<int> slotsV = busySlots.at(x);
        for (int y = 0; y < slotsV.size(); y++) {
            if (m_zdebug) log("slot["+QString::number(x)+"] pos["+QString::number(y)+"] " + QString::number(slotsV.at(y)));
        }
    }
...
bool MainWindow::setFreeFreq() {
    if (m_zdebug) log("setFreeFreq()");
    if (busySlots.size() < 2) {
        if (m_zdebug) log("Not enough data.");
        return false;
    }
  ...
  if (newTxFreq == 0) {
    ...
    if (m_zdebug) log("No free frequencies.");
    return false;
}
